...
public class LeoEngine extends SurfaceView implements Runnable {
     ...
     */
    public void step() {
        if (isRunning && cells.size() > 0 && isVisiable) {
     ...
     * @param rsID 资源ID
     */
    public void playSound(int rsID) {
        playSound(rsID, 1.0f, 1.0f, 0, 0, 1.0f);
    }
     ...
     */
    public void playSound(int rsID, final float leftVolume, final float rightVolume,
                          final int priority, final int loop, final float rate) {
        if (sounds.containsKey(rsID)) {
            sp.play(sounds.get(rsID), leftVolume, rightVolume, priority, loop, rate);
        } else {
